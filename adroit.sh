...
for round in {1..3}; do
  for data in ${datasets[@]}; do
    ...
    for i in {1..20};
    do
       CUDA_VISIBLE_DEVICES=0 python scripts/plan.py --test_planner beam_prior --dataset $data --exp_name $name-$round --suffix $i --beam_width 10 --b_percent 0.5 --n_expand 2 --n_actions 4 --action_percent 0.5 --pw_alpha 0 --mcts_itr 100
    done
  done
done
