...
#     dataloader = torch.nsutils.data.DataLoader(dataset, shuffle=True, pin_memory=True, **kwargs)
#     return dataloader
...
def qlearning_dataset_with_timeouts_training(env, dataset=None, terminate_on_end=False, disable_goal=False, **kwargs):
    if dataset is None:
        dataset = env.get_training_dataset(**kwargs)
    ...
    #print(dataset.keys())
    # Create or update the 'timeouts' field in the dataset
    if 'timeouts' not in dataset:
        # Ensure terminals are numpy arrays
        terminals = np.array(dataset['terminals'])
        N = len(dataset['observations'])

        # Find the indices of 'True' values in the 'terminals' list
        terminal_indices = np.where(terminals)[0]

        episode_durations = np.diff(np.concatenate(([0], terminal_indices + 1, [N])))


        dataset['timeouts'] = np.zeros(N, dtype=np.int8)

        # Mark timeouts in the dataset
        for i in range(len(episode_durations)):
            duration = episode_durations[i]
            start_index = -1 if i == 0 else terminal_indices[i - 1]

            # Debug print to check episode durations
            #print(f"Episode {i + 1} start index: {start_index}, duration: {duration}")

            if duration > 1000:
                for offset in range(1000, duration, 1000):
                    timeout_index = start_index + offset
                    if timeout_index < N:
                        dataset['timeouts'][timeout_index] = 1
                    #    print(f"Timeout set at index: {timeout_index}")
                    else:
                        print(f"Timeout index {timeout_index} out of bounds (N={N})")


    episode_step = 0
    #true_indexes = [index for index, value in enumerate(dataset["timeouts"]) if value]
    #print(true_indexes)
    for i in range(N-1):
