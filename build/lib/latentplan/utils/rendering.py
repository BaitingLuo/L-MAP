...
class Renderer:
    ...
    def render(self, observation, dim=256, render_kwargs=None):
        ...
        if render_kwargs is None:
            render_kwargs = {
                'trackbodyid': 2,
                'distance': 3,
                'lookat': [0, -0.5, 1],  # Default values
                'elevation': -20
            }

        # Dynamically update the 'lookat' parameter based on the Hopper's x-coordinate
        # Adjust the index if necessary based on your state structure
        hopper_x_position = observation[0]
        render_kwargs['lookat'][0] = hopper_x_position
    ...
    def render_plan(self, savepath, sequence, state, fps=30, save=True):
        ...
        sequence = to_np(sequence)
        ## compare to ground truth rollout using actions from sequence
        actions = sequence[:-1, self.observation_dim+1 : self.observation_dim + 1 + self.action_dim]
        rollout_states = rollout_from_state(self.env, state, actions)
        ...
        mse = ((sequence[:, 1:self.observation_dim+1] - rollout_states) ** 2).mean()
    ...
    def render_matplotlib(self, observation):
        img = self.render(observation)
        plt.imshow(img)
        plt.pause(0.0000001)  # Pause to update the figure
...
class KitchenRenderer:
    ...
    def render_rollout(self, savepath, sequence, state, **video_kwargs):
        sequence = sequence[:, 1:self.observation_dim+1]
        images = self(sequence) #np.stack(states, axis=0))
        save_video(savepath, images, **video_kwargs)
